...
//-------------- function to fliter input------------- 
    function validateStudentData($studentData) {      
        ...
        if (empty($studentData['name']) || empty($studentData['gender']) || empty($studentData['birth']) ||
        ...
        if (strlen($password) < 8) {
            return ['status' => 'error', 'message' => $password];
        ...
        if (isset($studentData['submitButtonClicked'])) {
            ...
            if ($validationResult['status'] === 'error') {
                echo json_encode(['status' => 'error', 'message' => $validationResult['message']]);
                    ...
                    $sql1 = "SELECT * FROM instructor WHERE email ='$email'";
                    $result = $conn->query($sql1);
                    $sql2 = "SELECT * FROM instructor WHERE Instructor_name ='$name'";
                    ...
                    if ($result->num_rows > 0 || $result2->num_rows > 0) {
                        echo json_encode(['status' => 'error', 'message' => ' مسجل مسبق ']);
                            ...
                            $sql = "INSERT INTO instructor (Instructor_name, Instructor_birth,phone,gender,data_join,email,password) VALUES ('$name', '$birth','$phone', '$gender','$data_join', '$email', '$password')";
...
// -------------display data student---------
        if (isset($studentData['diplayDataSend'])) {
                ...
                if (isset($studentData['searched']) && $studentData['searched']!==""){
                    ...
                    $sql = "SELECT * FROM instructor WHERE `instructor`.`Instructor_name` LIKE '%$searchTerm%'";
                }
                else{
                    $sql = "SELECT * FROM instructor ORDER BY 'Instructor_name'";
                }
                $result = $conn->query($sql);
                $number=1;
                while ($row = $result->fetch_assoc()) {
                    $table.='<tr>
                    <td><input class="form-check-input delete-checkbox" type="checkbox" value="'.$row['InstructorID'].'"  ></td>
                    <th scope="row">'.$number.'</th>
                    <td>' . $row['Instructor_name'] . '</td>
                    <td>' . $row['gender'] . '</td>
                    <td>' . $row['Instructor_birth'] . '</td>
                    <td>' . $row['email'] . '</td>
                    <td>' . $row['phone'] . '</td>
                    <td>' . $row['data_join'] . '</td>
                    <td><a  class="btn btnEdat"  data-bs-toggle="modal" data-bs-target="#editeStudent" onclick="GetData('.$row['InstructorID'].')"><i class="fa-solid fa-pen-to-square"></i></a></td>
                    <td><a  class="btn btnDelete"  onclick="DeleteUser('.$row['InstructorID'].')"><i class="fa-solid fa-trash-can"></i></a></td>
                    </tr>';
                    $number+=1;
                }
                $table.='</table>';
...
//------------ delet student ------------
        if (isset($studentData['delete'])) {
            ...
            $sql = "DELETE FROM instructor WHERE `instructor`.`InstructorID` = $IDstuden";
                     if ($conn->query($sql) === TRUE) {
...
//---------- get data student to update inf---------
        if (isset($studentData['studentids'])) {
                ...
                $sql = "SELECT * FROM instructor WHERE `instructor`.`InstructorID` = $ID";
                $result = $conn->query($sql);
       ...
       if (isset($studentData['idtoUpdate'])) {
            ...
            if ($validationResult['status'] === 'error') {
                echo json_encode(['status' => 'error', 'message' => $validationResult['message']]);
                    ...
                    $sql44="update instructor set Instructor_name='$name',gender='$gender',email='$email',password='$password',phone='$phone',Instructor_birth='$birth' WHERE InstructorID =$ID";
  ...
  if (isset($studentData['selectedRecord'])) {
        ...
        if (!empty($studentData['selectedRecord'])) {
            // تحويل القيم المحددة إلى مجموعة من الأرقام المفصولة بفواصل (1, 2, 3, ...)
                    ...
                    $sql = "DELETE FROM instructor WHERE InstructorID  IN ($recordIDs)";
